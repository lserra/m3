...
def include_user(s_user):
    """
    # função que cria a barra de menu/navegação com o nome do usuário que está acessando o sistema
    # a página em si é armazenada em um arquivo separado em "views/user.html"
    # e o elemento <$usuario> é substituído quando necessário
    :param s_user: "laercio.serra@gmail.com"
    :return: "laercio.serra@gmail.com"
    """
    with open('../views/user.html') as menuf:
        menu_text = menuf.read()

    menu_user = Template(menu_text)

    return menu_user.substitute(usuario=s_user)


def include_form_reg():
    """
    # função que cria o form de registro (cadastro) do associado
    # a página em si é armazenada em um arquivo separado em "views/formreg.html"
    :return:
    """
    with open('../views/formreg.html') as formf:
        form_text = formf.read()

    formreg = Template(form_text)

    return formreg.substitute()
...
def include_messages(s_type, s_msg):
    ...
    if s_type == '1':
        s_classe = "alert alert-error"
        s_desc = 'ERRO:'
    elif s_type == '2':
        s_classe = "alert alert-success"
        s_desc = 'SUCESSO:'
    elif s_type == '3':
        s_classe = "alert alert-info"
        s_desc = 'INFORMAÇÃO:'
    elif s_type == '4':
        s_classe = "alert"
        s_desc = 'ATENÇÃO:'
...
def include_data_table(rsdatatable):
    ...
    return formv.substitute(data_table=s_datatable)
