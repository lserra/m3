...
s_cdel = form_data.getvalue('cd')  # pega o valor do campo customer a ser apagado
...
# exclui o customer do sistema
(cstr_deleted, s_errormsg_d) = golias.delete_cstr(s_cdel)


# retorna a lista de customer
(s_fields, s_dt_tb, s_errormsg_c) = golias.get_all_cstr(s_domain)


# renderiza a página 'cstr.html' para visualizar os customer do sistema
print mysf.include_start_response()
print (mysf.include_header())
print (mysf.include_user(s_domain, s_nameuser, str.lower(s_emailassoc), s_date))
print (mysf.include_logout())
print (mysf.include_div_s())
if cstr_deleted is True:
    print (mysf.include_messages('2', ' Customer deleted with success!'))
else:
...
print (mysf.include_pageheader('Customer ', ' List all customer'))
print (mysf.include_button_create_new_cstr(s_domain, s_nameuser, str.lower(s_emailassoc)))
if s_dt_tb is None:
    print (mysf.include_data_table(s_fields))
else:
    print (mysf.include_dt_tb_enable_cstr(s_domain, s_nameuser, str.lower(s_emailassoc), s_fields, s_dt_tb))
print (mysf.include_pagination())
print (mysf.include_delete_cstr())
