...
form_data = cgi.FieldStorage()  # obter os dados de login do customer


s_cstr = form_data.getvalue('customer')  # pega o valor do campo customer
i_cstr = form_data.getvalue('id_cstr')  # pega o valor do campo id_cstr
...
n_field = []
s_f_msg = None
# valida se o campo customer foi informado pelo customer
if s_cstr is None:
    s_field = 0
    n_field.append('C')
...
if s_field != 0:
    ...
    if cstr_edited is True:
        ...
        print (mysf.include_pageheader('Customer ', ' Edit customer'))
        print (mysf.include_form_ecstr(s_cstr_new, i_cstr, s_domain, s_nameuser, str.lower(s_emailassoc)))
        print (mysf.include_div_e())
        print (mysf.include_footer())
    else:
        # renderiza a página 'ucstr.html' com a mensagem do erro para verificação e tratamento
        print mysf.include_start_response()
        print (mysf.include_header())
        print (mysf.include_user(s_domain, s_nameuser, str.lower(s_emailassoc), s_date))
        print (mysf.include_logout())
        ...
        print (mysf.include_pageheader('Customer ', ' Edit customer'))
        print (mysf.include_form_ecstr(s_cstr, i_cstr, s_domain, s_nameuser, str.lower(s_emailassoc)))
        print (mysf.include_div_e())
        print (mysf.include_footer())
else:
    # renderiza a página 'ucstr.html' com a mensagem do erro para verificação e tratamento
    ...
    print (mysf.include_pageheader('Customer ', ' Edit customer'))
    print (mysf.include_form_ecstr_err(s_cstr, i_cstr, s_domain, s_nameuser, str.lower(s_emailassoc), n_field))
    print (mysf.include_div_e())
    print (mysf.include_footer())