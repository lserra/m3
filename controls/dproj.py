...
s_pdel = form_data.getvalue('pd')  # pega o valor do campo project a ser apagado
...
# exclui o project do sistema
(proj_deleted, s_errormsg_d) = golias.delete_proj(s_pdel)


# retorna a lista de project
(s_fields, s_dt_tb, s_errormsg_c) = golias.get_all_proj(s_domain)


# renderiza a página 'proj.html' para visualizar os projects do sistema
print mysf.include_start_response()
print (mysf.include_header())
print (mysf.include_user(s_domain, s_nameuser, str.lower(s_emailassoc), s_date))
print (mysf.include_logout())
print (mysf.include_div_s())
if proj_deleted is True:
    print (mysf.include_messages('2', ' Project deleted with success!'))
else:
...
print (mysf.include_pageheader('Project ', ' List all project'))
print (mysf.include_button_create_new_proj(s_domain, s_nameuser, str.lower(s_emailassoc)))
if s_dt_tb is None:
    print (mysf.include_data_table(s_fields))
else:
    print (mysf.include_dt_tb_enable_proj(s_domain, s_nameuser, str.lower(s_emailassoc), s_fields, s_dt_tb))
print (mysf.include_pagination())
print (mysf.include_delete_proj())
