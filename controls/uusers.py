...
# se todos os campos foram preenchidos, então realiza a alteração dos dados do usuário no sistema
if s_field != 0:
    s_name_eduser = str.strip(s_fname_eduser) + ' ' + str.strip(s_lname_eduser)

    # retorna os dados do associado editado
    ...
    if user_edited is True:
        # renderiza a página 'eusers.html' para continuar com a edição do usuário no sistema
        ...
        # renderiza a página 'eusers.html' com a mensagem do erro para verificação e tratamento
        print mysf.include_start_response()
        print (mysf.include_header())
        print (mysf.include_user(s_domain, s_nameuser, str.lower(s_emailassoc), s_date))
        print (mysf.include_logout())
