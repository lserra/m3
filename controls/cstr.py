...
if s_profile != 'S':  # somente supervisor (S) possui acesso a este módulo
    # renderiza a página 'cstr.html' com a mensagem de que o usuário não possui permissão
    ...
    (s_fields, s_dt_tb, s_errormsg) = golias.get_all_cstr(s_domain)

    # renderiza a página 'cstr.html' para visualizar a lista de customers
    print mysf.include_start_response()
    print (mysf.include_header())
    print (mysf.include_user(s_domain, s_nameuser, str.lower(s_emailassoc), s_date))
    print (mysf.include_logout())
    print (mysf.include_div_s())
    print (mysf.include_pageheader('Customer ', ' List all customers'))
